...
public interface LotteryEntryRepository extends JpaRepository<LotteryEntry, Long> {
    ...
    @Query("SELECT DISTINCT le.lotteryEvent FROM LotteryEntry le WHERE COALESCE(le.userId, le.ticket.userId) = :userId ORDER BY le.lotteryEvent.endDateTime DESC")
    List<LotteryEvent> findDistinctLotteryEventsByUserId(@Param("userId") Long userId);

    @Query("SELECT DISTINCT le.lotteryEvent FROM LotteryEntry le WHERE COALESCE(le.userId, le.ticket.userId) = :userId AND le.lotteryEvent.endDateTime < :currentDateTime ORDER BY le.lotteryEvent.endDateTime DESC")
    ...
    @Query("SELECT COUNT(DISTINCT COALESCE(e.userId, e.ticket.userId)) FROM LotteryEntry e WHERE e.lotteryEvent = :event")
